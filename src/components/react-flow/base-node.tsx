...
export function BaseNode({ className, status, ...props }: BaseNodeProps) {
	return (
		<div
			...
			// biome-ignore lint/a11y/noNoninteractiveTabindex: biome complains about tabIndex usage
			tabIndex={0}
			{...props}
		>
			{props.children}
			...
			{status === "success" && (
				<CheckCircle2 className="absolute right-0.5 bottom-0.5 size-2 text-emerald-600 stroke-3" />
			)}
			{status === "loading" && (
				<Loader2 className="absolute animate-spin -right-0.5 -bottom-0.5 size-2 text-blue-700 stroke-3" />
			)}
		</div>
	);
}
